...
window.addEventListener("DOMContentLoaded", async (event) => {
  ...
  try {
    let response = await axios.get("http://localhost:5000/expense/getAllUrl", {
...
*/
    ...
    let response = await axios.get("http://localhost:5000/expense/download", {
      headers: { Authorization: token },
    });
...
function showUrlOnScreen(data) {
  ...
  //console.log(data.fileURL)
  let child = `<li class="list" >
        <a href="${data.fileURL}" class="expense-info">${listno + 1} ${
